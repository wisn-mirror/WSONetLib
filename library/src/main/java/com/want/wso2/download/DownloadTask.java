...
public class DownloadTask implements Runnable {
    ...
    public DownloadTask(String tag, Request<File, ? extends Request> request) {
        ...
        progress.tag = tag;
        progress.folder = Download.getInstance().getFolder();
        progress.url = request.getBaseUrl();
        progress.status = Progress.NONE;
        progress.totalSize = -1;
        progress.request = request;

        executor = Download.getInstance().getThreadPool().getExecutor();
    ...
    public void start() {
        if (Download.getInstance().getTask(progress.tag) == null || DownloadManager.getInstance().get(progress.tag) == null) {
    ...
    public DownloadTask remove(boolean isDeleteFile) {
        ...
        DownloadTask task = Download.getInstance().removeTask(progress.tag);
