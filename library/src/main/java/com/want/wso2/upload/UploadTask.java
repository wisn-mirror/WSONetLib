...
import com.want.wso2.base.ProgressRequestBody;
...
public class UploadTask<T> implements Runnable {
    ...
    public UploadTask(String tag, Request<T, ? extends Request> request) {
        ...
        executor = Upload.getInstance().getThreadPool().getExecutor();
    ...
    public UploadTask<T> start() {
        if (Upload.getInstance().getTask(progress.tag) == null || UploadManager.getInstance().get(progress.tag) == null) {
    ...
    public UploadTask<T> remove() {
        ...
        UploadTask<T> task = (UploadTask<T>) Upload.getInstance().removeTask(progress.tag);
